...
<body>
    <div class="main">
        <table>
            ...
            <?php foreach($orderList as $vkey=>$orderItem){?>
                <tr <?php if($vkey%2){echo 'style="background-color:#f5fbfb"';}?> onmouseover="bg(this);" onmouseout="obg(this)">
                    <td class="fields"><?php echo $orderItem['detailDesc'];?></td>
                    <td><?php echo date('Y-m-d H:i:s',$orderItem['addTime']);?></td>
                    <td><?php echo date('Y-m-d H:i:s',$orderItem['updateTime']);?></td>
                    <td><?php echo $orderItem['categoryInfo']['categoryName'];?></td>
                    <td><?php echo $orderItemStatus[$orderItem['status']];?></td>
                    <td><a href="/index/workItemInfo?workItemOrderId=<?php echo $orderItem['id'];?>">查看详情</a> </td>
                </tr>
            <?php } ?>
        </table>
